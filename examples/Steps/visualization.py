...
z_x   = df.z_x
z0_x   = df.a_r_x * np.sin(df.p_r_x)
z_u_x = df.a_u_x * np.sin(df.p_u_x)
z_e_x = df.a_e_x * np.sin(df.p_e_x)
...
z_y   = df.z_y
z0_y   = df.a_r_y * np.sin(df.p_r_y)
z_u_y = df.a_u_y * np.sin(df.p_u_y)
z_e_y = df.a_e_y * np.sin(df.p_e_y)
